...
async function debugPath(pathStr) {
  try {
    const names = await listNamesRaw(pathStr);
    const numeric = names.map((n) => { const m = /^\s*0*(\d+)/.exec(n); return m ? +m[1] : null; }).filter((x) => x != null).sort((a, b) => a - b);
    return { path: pathStr, count: names.length, highestNumber: numeric[numeric.length - 1] || null, names: names.slice(0, 80) };
  } catch (e) { return { path: pathStr, error: e.message }; }
}

module.exports = { attachPdfUrls, streamPdf, resolvePdf, hasPdf, debugCategory, debugPath, FOLDERS };
