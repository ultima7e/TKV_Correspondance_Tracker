...
// For each letter, set letterUrl to the streaming endpoint when the phonebook
// has a PDF for it. Reads one small index file (no listing cap). Never throws.
async function attachPdfUrls(letters) {
  const idx = (await loadIndex()) || {};
  for (const l of letters) {
    const n = tailNum(l.letterNumber);
    if (n && idx[l.category] && idx[l.category][n]) {
...
module.exports = { attachPdfUrls, streamPdf, resolvePdf, hasPdf, debugCategory, debugPath, debugGet, FOLDERS };
