...
const SHEETS = {
  ...
  // Miscellaneous letters have no letter number and a simpler schema
  // (Date / From / Subject / Attachment / Doc Type / Tags). Resolved fuzzily
  // by name below, so a slightly different tab spelling still matches.
...
const liveZone = {
  ...
  miscellaneous: () => true,   // no letter number — every misc row is live-driven by the Excel
};
...
// Miscellaneous letters: no number, simpler schema. Keyed by subject (+date) so
// the id is stable across loads. Kept read-only like the other Excel letters.
function slug(s) { return String(s || '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 60); }
function mapMiscRow(row, col) {
  const g = (name) => { const i = col(name); return i >= 0 ? row[i] : null; };
  const subject = text(g('subject'));
  const date = isoDate(g('date')) || isoDate(g('send date'));
  return {
    id: 'xlsx:miscellaneous:' + slug(subject) + (date ? ':' + date : ''),
    createdAt: Date.parse(date) || 0,
    category: 'miscellaneous',
    letterNumber: '',            // misc letters are identified by subject, not a number
    sendDate: date,
    hardCopyDate: '',
    subject,
    from: text(g('from')),
    employerRefs: [], engineerRefs: [], sinohydroRefs: [], replyRefs: [], engineerReplies: [], sinoReplies: [],
    letterUrl: '', attachments: [],
    department: normDept(g('department')),
    status: text(g('status')),
    cc: '', remarks: text(g('remarks')),
    attachment: text(g('attachment')),
    docTypes: splitTags(g('doc type')),
    locations: splitTags(g('location')),
    tags: splitTags(g('tags')),
...
// Resolve a sheet by exact name, then case/space-insensitively, then (for the
// misc tab) any sheet whose name contains "miscellaneous" — tolerant of small
// naming differences like "Miscellaneous Letter" vs "Miscellaneous Letters".
function findSheet(wb, wanted) {
  ...
  if (target.includes('miscellaneous')) return wb.SheetNames.find((n) => norm(n).includes('miscellaneous')) || null;
  return null;
}

// Find the header row: the first of the first few rows that contains a "subject"
// cell. Misc puts headers on row 0; the numbered sheets on row 1 (title above).
function findHeaderRow(mat) {
  for (let i = 0; i < Math.min(mat.length, 6); i++) {
    const cells = (mat[i] || []).map((c) => (c == null ? '' : String(c).toLowerCase()));
    if (cells.some((c) => c.includes('subject'))) return i;
  }
  return 1;
}
...
function parseWorkbook(buf) {
  ...
  const out = [];
  for (const [category, wanted] of Object.entries(SHEETS)) {
    const sheetName = findSheet(wb, wanted);
    if (!sheetName) continue;
    const ws = wb.Sheets[sheetName];
    const mat = XLSX.utils.sheet_to_json(ws, { header: 1, defval: null });
    // Keep the three numbered sheets on their known layout (header on row 1);
    // auto-detect only for miscellaneous, whose header sits on row 0.
    const hdr = category === 'miscellaneous' ? findHeaderRow(mat) : 1;
    const H = (mat[hdr] || []).map((h) => (h == null ? '' : String(h).replace(/\s+/g, ' ').trim().toLowerCase()));
    const col = (n) => H.indexOf(n.toLowerCase());
    for (let r = hdr + 1; r < mat.length; r++) {
      const row = mat[r] || [];
      if (category === 'miscellaneous') {
        const L = mapMiscRow(row, col);
        if (!L.subject) continue;         // subject is the required field here
        out.push(L);
        continue;
      }
...
// Admin diagnostic: fetch the master Excel and report its real sheet names, how
// the Miscellaneous tab resolves, its detected header row + headers, and how many
// misc letters parse. Lets us confirm the live workbook layout without guessing.
async function debugSheets() {
  const { NUTSTORE_USER, NUTSTORE_PASSWORD } = process.env;
  if (!NUTSTORE_USER || !NUTSTORE_PASSWORD) return { error: 'no Nutstore creds' };
  const headers = { Authorization: 'Basic ' + Buffer.from(`${NUTSTORE_USER}:${NUTSTORE_PASSWORD}`).toString('base64') };
  const res = await fetch(DAV_BASE + encPath(EXCEL_PATH), { headers });
  if (!res.ok) return { error: `Nutstore ${res.status} ${res.statusText}`, excelPath: EXCEL_PATH };
  const buf = Buffer.from(await res.arrayBuffer());
  const wb = XLSX.read(buf, { type: 'buffer', cellDates: true });
  const miscName = findSheet(wb, SHEETS.miscellaneous);
  let miscHeaderRow = null, miscHeaders = null;
  if (miscName) {
    const mat = XLSX.utils.sheet_to_json(wb.Sheets[miscName], { header: 1, defval: null });
    miscHeaderRow = findHeaderRow(mat);
    miscHeaders = (mat[miscHeaderRow] || []).map((c) => (c == null ? '' : String(c)));
  }
  const parsed = parseWorkbook(buf);
  return {
    excelPath: EXCEL_PATH,
    sheetNames: wb.SheetNames,
    miscResolvedAs: miscName,
    miscHeaderRow,
    miscHeaders,
    miscLettersParsed: parsed.filter((l) => l.category === 'miscellaneous').length,
  };
}

module.exports = { readExcelLive, parseWorkbook, mergeExcelLive, mergeVocab, normalizeCaseAgainst, liveZone, debugSheets };
