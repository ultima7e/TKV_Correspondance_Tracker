...
function mergeExcelLive(jsonLetters, excelLetters) {
  const excelKeys = new Set(excelLetters.map((l) => l.category + '|' + String(l.letterNumber).replace(/\s/g, '')));
  const kept = jsonLetters.filter((l) => {
    ...
    return !excelKeys.has(l.category + '|' + String(l.letterNumber).replace(/\s/g, '')); // in zone -> Excel wins if present
  });
  return kept.concat(excelLetters);
}
